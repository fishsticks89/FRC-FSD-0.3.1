...
class RobotLocomotionEnv(gym.Env):
    ...
    def get_state(self):
        ...
        return [ag_pos[0], ag_pos[1], ag_vel[0], ag_vel[1]]
        return [ag_pos[0], ag_pos[1], ag_vel[0], ag_vel[1]]
    ...
    def vel_to_target(self):
        ag_vel, ag_angular_vel = p.getBaseVelocity(self.ag_body)
        ag_pos, _ = p.getBasePositionAndOrientation(self.ag_body)
        
        # Get vector from agent to target
        vec_to_target = np.array([0, 0]) - np.array([ag_pos[0], ag_pos[1]])
        
        # Normalize to unit vector
        vec_to_target = vec_to_target / np.linalg.norm(vec_to_target)
        
        # Get dot product with velocity
        return np.dot(np.array([ag_vel[0], ag_vel[1]]), vec_to_target)
    def vel_to_target(self):
        ag_vel, ag_angular_vel = p.getBaseVelocity(self.ag_body)
        ag_pos, _ = p.getBasePositionAndOrientation(self.ag_body)
    ...
    def get_agent_reward(self):
        ...
        return (time_penalty *0.2 + dist_to_target_penalty * 0.2) / self.max_steps * self.success_reward
        return (time_penalty *0.2 + dist_to_target_penalty * 0.2) / self.max_steps * self.success_reward
...
policy_net_ag.load_state_dict(torch.load("./0.3.1.2/policy_net_agent.pth", map_location=device))
